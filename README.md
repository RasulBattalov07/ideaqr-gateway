...
The interface is themed as a **secure terminal / control room**: a deep-ink base,
Kazakh-sky **cyan** as the primary accent and **gold** as the trust accent, with
*Golos Text* for Cyrillic UI and *JetBrains Mono* for codes and UUIDs. The signature
element is the **governance pipeline tracker**, which lights
Личность → Запрос → Решение → Действие → История with the real generated UUIDs on every
scan and every object creation.
