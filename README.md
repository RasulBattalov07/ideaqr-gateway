...
> Архитектура **мульти-тенантная** (жёсткая изоляция организаций); предзаполненные демо-сценарии: **Медицина**, **P2P**, **Инфраструктура**, **Право/полиция**, **Услуги ЖКХ**.
...
mvn clean test                 # весь сьют (67 тестов)
...
- **Сброс БД:** останови приложение и удали папку `./data/` — при следующем старте `DataSeeder` пересоздаст всё (8 аккаунтов, орг-ции, объекты, цифровые досье).
...
### 📁 Папка `data/`
Локальная runtime-БД H2 (`data/ideaqr.mv.db`) — **создаётся автоматически** при первом старте; `DataSeeder` заполняет её демо-данными (4 организации-тенанта, 8 аккаунтов, демо-объекты, личность-визитка, цифровые досье граждан). Это **не** каталог SQL-сидов — сами демо-данные заданы в коде: `DataSeeder.java`, `RegistryClient.java`, `CitizenDossierService.java` и миграции Flyway `db/migration/V1…V9`. Бинарный файл БД в git не попадает; отслеживается только `data/_INFO.md` с описанием для заказчика. **Сброс к чистому демо:** удалить `data/ideaqr.mv.db` и перезапустить.
...
| Миграции | Flyway `V1…V9` (V9 — Java-миграция: расширение CHECK-констрейнтов под POLICE/LEGAL/SERVICE_ORDER; схема под `ddl-auto=validate` — fail-fast при дрейфе) |
...
**Организации (тенанты):** `IDEAQR Retail` (RETAIL) · `Городская больница` (MEDICAL) · `АО «Астана-РЭК»` (INFRASTRUCTURE) · `Департамент полиции Астаны` (GOVERNMENT).
...
## 🧪 Тесты (67, все зелёные)
Ключевые инварианты: `ContextAwareScanTest` (Phase 2: полиция→досье, фармацевт→рецепты, врач→консент, гражданин→визитка, роль×режим, owner-override), `EgovOnboardingTest` (телефон→Расул Батталов→пакет+живая сессия, OTP 1234), `MedicalConsentTest` (консент-парадокс закрыт), `ValidationServiceTest` (политики + фармацевт), `GuestServiceMergeTest` (IDOR-safe merge), `ObjectLifecycleServiceTransferTest`, `PublicCardTest`, `AuditServiceChainTest` (tamper-evidence), `TenantIsolationTest`, `TenantHttpIsolationTest`, `ForeignKeyIntegrityTest`, `SecurityIntegrationTest`, `RateLimitingTest`, `UserManagementTest`, `PasswordLifecycleTest`. Все сценарии Phase 2 дополнительно проверены живым многоролевым прогоном (curl-смоук: eGov-регистрация, 4 контекста одного QR, консент, выдача рецепта, заявка ЖКХ, OTP-вход).
